...
- [4. ハンズオン: 教室のみんなでリアルタイムチャット（15分）](#4-ハンズオン-教室のみんなでリアルタイムチャット15分)
  ...
  - [進め方](#進め方)
  - [Step 1. ファイルを準備する（2分）](#step-1-ファイルを準備する2分)
  - [Step 2. 生の SSE をこの目で見る（3分）](#step-2-生の-sse-をこの目で見る3分)
...
```mermaid
flowchart LR
    s1["Step 1<br>📦 準備<br>（2分）"] --> s2["Step 2<br>👀 生の SSE を見る<br>（3分）"] --> s3["Step 3<br>📻 受信を作る<br>（7分）"] --> s4["Step 4<br>📮 送信を作る<br>（3分）"]
```
...
### Step 2. 生の SSE をこの目で見る（3分）

コードを書く前に、講義でやった「SSE = ただのテキストが流れる終わらないレスポンス」を確かめましょう。
...
確認できたらこのタブは閉じて OK です。ここからは、この生テキストを `EventSource` に受け取らせて、ちゃんとした画面にしていきます。
...
ここに、講義 3-2 でやった `EventSource` のコードを書きます。やることは2つ:
...
やることは1つ。**送信は SSE ではなく、普通の HTTP POST** です（講義 2-4 で話した「受信は SSE、送信は普通の HTTP」の形ですね）:
...
- **自動再接続を観察する**: PC の Wi-Fi を数秒切って戻し、開発者ツールの Network タブで `events` への接続が自動で張り直されるのを見てみる（講義 3-2 の図が本当に起きます）
